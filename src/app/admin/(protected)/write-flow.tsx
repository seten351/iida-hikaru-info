...
export function AppearanceEditor({
  ...
  return (
    <form action={dispatch} className="admin-write-form">
      ...
      <label>カテゴリ<select value={fields.category} onChange={(event) => update("category", event.target.value)}>{["テレビ", "ラジオ", "配信", "イベント", "その他"].map((item) => <option key={item}>{item}</option>)}</select></label>
